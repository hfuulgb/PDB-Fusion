...
def getmodel_multicnn():
    ...
    x2 = Convolution1D(
        64, 16, strides=1, padding="same", kernel_initializer="he_normal"
    )(x2)
    x2 = BatchNormalization()(x2)
    ...
    model = models.Model(inputs, x, name="PDB_MultipleScaleCnn_Model")
    return model
...
def get_dense_model():
    ...
    x2 = Convolution1D(
        32, 8, strides=1, padding="same", kernel_initializer="random_uniform"
    )(x)
    x2 = BatchNormalization()(x2)
    x2 = Activation("relu")(x2)
    x2 = MaxPooling1D(pool_size=3)(x2)
    ...
    x = Bidirectional(LSTM(32, return_sequences=True))(x2)
    x = Flatten()(x)
