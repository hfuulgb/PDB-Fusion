...
def get_CNN_BILSTM_model(cnn_size=4,filter=64,conv_width=7,max_pool_size=2,uselstm=1,lstm_size=32,dense_size=128,drop=0.2):   
    # 构建网络
    ...
    for drop_o in drop_out[:-1]:
        for cnn_layers in cnnmodel_array[:-1]:
            for use_blstm in uselstm[:-1]:
                ...
                fw_perf.writelines('cnn_layers=['+str(cnn_layers)+']--[conv_width=['+str(conv_width)+ ']\n')
                ...
                for index in range(1):
                    dataset = []
                    dataset = tool.read_seq_onehot("data/DNA_Pading_600_PDB1075")
                    # print(dataset.shape[0])
                    label = np.loadtxt("data/class_PDB1075")
                    # print(label.shape())
                    ...
                    for ((train, test), k) in zip(skf.split(dataset, label),
                        ...
                        model = get_CNN_BILSTM_model(cnn_size=cnn_layers,conv_width=7,uselstm=use_blstm,lstm_size=lstm_size2,drop=drop_o)
