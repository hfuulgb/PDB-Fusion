...
def get_CNN_BILSTM_model(
    ...
    for index in range(cnn_size):
        ...
        x = layers.MaxPooling1D(
            pool_size=max_pool_size, name="max_pooling1d_1" + str(index)
        )(x)
    ...
    if uselstm == 1:
        x = layers.Bidirectional(
            LSTM(lstm_size, return_sequences=True), name="bidirectional_6"
        )(x)
...
if __name__ == "__main__":
    ...
    max_width = []
    for i in np.arange(750, 2800, 50):
        max_width.append(i)

    for maxlen in max_width[:-1]:
            ...
            for cnn_layers in cnnmodel_array[:-1]:
                for use_blstm in uselstm[:-1]:
                    ...
                    for index in range(5):
                        dataset = []
                        dataset = tool.read_seq_onehot(
                            "data/DNA-Pading-DB14189-" + str(maxlen)
                        )
